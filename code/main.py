...
class AllSprites(pygame.sprite.Group):
	...
	def customize_draw(self):
		...
		self.offset.y=player.rect.centery-WINDOW_WIDTH/2

		display_surface.blit(self.bg,-self.offset)

		#basically sprites list goes into the lambda and lambda sorts the highest y to the lowest and returns it
...
# test_car=Car((100,200),all_sprites)
...
#long sprites
for long_file_name,long_pos_list in LONG_OBJECTS.items():
	path=f"../graphics/objects/long/{long_file_name}.png"
	surf=pygame.image.load(path).convert_alpha()
	for pos in long_pos_list:
...
while True:
	...
	#exercise:only update and draw all_sprites if the player is in a position that is greater than 1180

	if player.rect.top>1180:
